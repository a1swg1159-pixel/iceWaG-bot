...
| 群内只 @ 机器人（无文字） | 获取随机音击图片，失败则回退到 QQ 内置表情 | `src/plugins/random_face/` |
...
| **其他群** | `/help`、签到、猫猫币、二十一点、老虎机、随机音击图片，以及 `/chu`、`/mai`、`/takumi` 全部查分功能 |
...
| `/mai 更新 <机台扫码字符串>` | 私聊使用 `SGWCMAID...` 字符串拉取机台最新成绩并写入 LXNS |
| `/mai b50` | 生成 Best 50 图片（旧版本 35 + 当前版本 15） |

插件路径: `src/plugins/maimai_b50/`

> 💡 查分可在任意群使用；与 `/chu` 共用 LXNS OAuth，`bind` 和 `更新` 必须在私聊完成。扫码字符串只用于当次拉取，不保存到本地绑定文件；受机台接口限制，扫码同步可更新达成率和 DX 分，但不包含 FC/FS/PC 等详细字段。生成图片底部统一标注 `generated by iceWaG`。
