...
# LXNS OAuth：应用选择“无回调地址”，API 权限勾选 read_user_profile、read_player
LXNS_OAUTH_CLIENT_ID=你的OAuth应用ID
...
`read_user_profile` 和 `read_player`。不要申请 `read_user_token`、`write_player`、
邮箱或 OpenID 权限。将应用 ID 写入 `.env` 后重启 Bot。用户私聊发送
...
> 💡 查分可在任意群使用；与 `/chu` 共用 LXNS OAuth，`bind` 授权绑定请在私聊完成。生成图片底部统一标注 `generated by iceWaG`。
